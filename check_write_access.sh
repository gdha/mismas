...
progress_counter=-1
find "$WorkingDir" -type d | while IFS= read -r DIR
do
   printf "%s" "$DIR," >> "$filename"
   progress_counter=$(( progress_counter + 1 ))
   [[ $progress_counter -gt 3 ]] && progress_counter=0
   printf "\b${progress_chars[$progress_counter]}"  # print a dot to standard output for animation

   # remove everything that is not group related from getacl output and only keep 'rwx' entries
   $GETACL "$DIR" 2>/dev/null | grep -vE '(\#|default:|class:|owner:|user:|other:)' | grep rwx | while IFS=":" read -r f1 f2 f3
   do
