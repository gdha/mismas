...
# Find the most recently created rear workspace safely (avoid parsing ls)
ROOT_REAR=$(find /tmp -maxdepth 1 -name 'rear.*' -printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)

if [[ -z "$ROOT_REAR" ]] ; then
    echo "ERROR: Could not find a rear workspace under /tmp — did 'rear mkrescue' succeed?"
    exit 1
fi
